...
async def _parseData(bot_application: Application, current_time: int, sheets_creds):
...
async def _checkNotAnswered(bot_application: Application, current_time: int):
...
def RunParsing(bot_application: Application):
    asyncio.run(_runParsing(bot_application))


async def _runParsing(bot_application: Application):
